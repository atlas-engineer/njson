...
(define-test jget-errors ()
  (let ((object (decode (asdf:system-relative-pathname :njson "tests/test.json"))))
    (assert-error 'no-such-key (jget 20 object t))
    (assert-error 'non-indexable (jget 20 nil t))
    (assert-error 'non-indexable (jget 20 t t))
    (assert-error 'non-indexable (jget 20 :null t))
    (assert-error 'non-indexable (jget 20 :undefined t))
    (assert-error 'non-indexable (jget 20 200 t))
    (assert-error 'non-indexable (jget 20 200.3 t))
    (assert-error 'non-indexable (jget 20 "foo" t))
    (assert-error 'invalid-key (jget 20 (jget 9 object) t))
    (assert-error 'invalid-key (jget "bar" (jget 8 object) t))
    (assert-error 'invalid-pointer (jget #p"hello" object t))))
...
(define-test ensure ()
  ...
  (assert-error 'no-such-key (ensure-object "number" (njson:decode "{}")))
